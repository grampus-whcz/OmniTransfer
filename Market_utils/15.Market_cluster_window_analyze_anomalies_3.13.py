...
# === 新增：Market知识图谱构建函数 ===
def build_market_knowledge_graph(cluster_id, cluster_anomalies, output_dir, rca_analyzer):
    """
    为单个聚类构建Market知识图谱（参照Bank逻辑适配）
    
    Args:
        cluster_id: 聚类ID
        cluster_anomalies: 该聚类的异常列表
        output_dir: 输出目录
        rca_analyzer: Market RCA分析器实例
    ...
    # 初始化知识图谱结构
    kg_data = {
        "cluster_id": f"market_cluster_{cluster_id}",
        "creation_time": datetime.now(BEIJING_TZ).isoformat(),
        "total_anomalies": len(cluster_anomalies),
        "time_span": {
            "start": min([a['ts'] for a in cluster_anomalies]),
            "end": max([a['ts'] for a in cluster_anomalies]),
            "duration_sec": max([a['ts'] for a in cluster_anomalies]) - min([a['ts'] for a in cluster_anomalies])
        },
        "nodes": [],
        "relationships": []
    }
    ...
    # 节点ID计数器
    node_id_counter = 0
    node_id_mapping = {}  # 实体名 → 节点ID
    
    # 1. 构建实体节点（适配Market实体类型）
    entity_types = {
        "frontend": "OS",
        "adservice": "OS",
        "cartservice": "OS",
        "checkoutservice": "OS",
        "currencyservice": "OS",
        "recommendationservice": "OS",
        "productcatalogservice": "OS",
        "shippingservice": "OS",
        "emailservice": "OS",
        "paymentservice": "OS",
        "node": "OS",
        "istio-egressgateway": "OS",
        "istio-ingressgateway": "OS"
    }
    
    # 提取所有实体（原始+清洗后）
    all_entities = set()
    entity_clean_mapping = {}
    
    for anomaly in cluster_anomalies:
        original_entity = anomaly['entity']
        cleaned_entity = rca_analyzer._clean_entity_name(original_entity)
        all_entities.add(original_entity)
        all_entities.add(cleaned_entity)
        entity_clean_mapping[original_entity] = cleaned_entity
    
    # 创建实体节点
    for entity in all_entities:
        # 确定节点类型
        cleaned_entity = rca_analyzer._clean_entity_name(entity)
        layer = rca_analyzer._identify_service_layer(cleaned_entity)
        node_type = entity_types.get(layer, "unknown")
        
        # 判断是否为主实体
        is_main_entity = not (any(char.isdigit() for char in cleaned_entity) and '-' in cleaned_entity)
        
        node_id = f"node_{node_id_counter}"
        node_id_counter += 1
        node_id_mapping[entity] = node_id
        ...
        node_data = {
            ...
            "properties": {
                "entity_id": entity,
                "cleaned_entity_id": cleaned_entity,
                "is_main_entity": is_main_entity,
                "service_layer": layer if layer else "unknown",
                "layer_weight": rca_analyzer.chain_weights.get(layer, 0.5),
    ...
    anomaly_attrs = set()
    for anomaly in cluster_anomalies:
        anomaly_attrs.add(anomaly['attribute'])
    ...
    for attr in anomaly_attrs:
        ...
        if any(keyword in attr_lower for keyword in ["error", "exception", "fail"]):
            fault_type = "ERROR"
        elif any(keyword in attr_lower for keyword in ["latency", "delay", "timeout"]):
            fault_type = "LATENCY"
        elif any(keyword in attr_lower for keyword in ["cpu", "memory", "disk", "resource"]):
            fault_type = "RESOURCE_EXHAUSTION"
        elif any(keyword in attr_lower for keyword in ["network", "connection", "tcp"]):
            fault_type = "NETWORK_ISSUE"
        elif any(keyword in attr_lower for keyword in ["log", "warn", "info"]):
            fault_type = "LOG_PATTERN"
        
        node_id = f"node_{node_id_counter}"
        node_id_counter += 1
        node_id_mapping[f"attr_{attr}"] = node_id
        ...
        node_data = {
            ...
            "properties": {
                ...
                "description": f"Anomaly attribute: {attr}",
                "created_at": datetime.now(BEIJING_TZ).isoformat()
            }
        }
        kg_data["nodes"].append(node_data)
    ...
    for node in kg_data["nodes"]:
        if node["label"] == "AnomalyAttribute":
            fault_types.add(node["properties"]["fault_type"])
    
    for fault_type in fault_types:
        node_id = f"node_{node_id_counter}"
        node_id_counter += 1
        node_id_mapping[f"fault_{fault_type}"] = node_id
        ...
        node_data = {
            ...
            "properties": {
                ...
                "description": f"Fault type: {fault_type}",
                "created_at": datetime.now(BEIJING_TZ).isoformat()
            }
        }
        kg_data["nodes"].append(node_data)
    ...
    # 4. 构建关系
    # 4.1 HAS_ANOMALY 关系（实体→属性）
    anomaly_relation_counter = defaultdict(int)
    for anomaly in cluster_anomalies:
        entity = anomaly['entity']
        attr = anomaly['attribute']
        
        if entity not in node_id_mapping or f"attr_{attr}" not in node_id_mapping:
            continue
        
        anomaly_relation_counter[(entity, attr)] += 1
        
        relation_data = {
            "type": "HAS_ANOMALY",
            "source": node_id_mapping[entity],
            "target": node_id_mapping[f"attr_{attr}"],
            "properties": {
                "timestamp": anomaly['ts'],
                "time_str": ts_to_beijing_str(anomaly['ts']),
                "anomaly_type": anomaly['type'],
                "occurrence_count": anomaly_relation_counter[(entity, attr)],
                "created_at": datetime.now(BEIJING_TZ).isoformat()
            }
        }
        kg_data["relationships"].append(relation_data)
    
    # 4.2 HAS_ATTRIBUTE 关系（实体→属性）
    for anomaly in cluster_anomalies:
        entity = anomaly['entity']
        attr = anomaly['attribute']
        ...
        relation_data = {
            ...
            "target": node_id_mapping[f"attr_{attr}"],
            "properties": {
                "attribute_type": anomaly['type'],
                "first_occurrence_ts": anomaly['ts'],
                "first_occurrence_time": ts_to_beijing_str(anomaly['ts']),
                "created_at": datetime.now(BEIJING_TZ).isoformat()
            }
        }
        kg_data["relationships"].append(relation_data)
    
    # 4.3 MAPS_TO_FAULT 关系（属性→故障类型）
    for node in kg_data["nodes"]:
        if node["label"] == "AnomalyAttribute":
            attr_id = node["properties"]["attribute_id"]
            fault_type = node["properties"]["fault_type"]
            
            if f"attr_{attr_id}" not in node_id_mapping or f"fault_{fault_type}" not in node_id_mapping:
                continue
            
            relation_data = {
                "type": "MAPS_TO_FAULT",
                "source": node_id_mapping[f"attr_{attr_id}"],
                "target": node_id_mapping[f"fault_{fault_type}"],
                "properties": {
                    "confidence": 1.0,
                    "created_at": datetime.now(BEIJING_TZ).isoformat()
                }
    ...
    # 4.4 TOPOLOGY_DEPENDS_ON 关系（实体→实体，基于Market基础拓扑）
    # 获取拓扑图
    G = rca_analyzer.build_topology_graph(cluster_anomalies)
    
    # 遍历拓扑边
    for u, v in G.edges:
        # 检查实体是否在节点映射中
        u_clean = rca_analyzer._clean_entity_name(u)
        v_clean = rca_analyzer._clean_entity_name(v)
        
        source_node_id = None
        if u in node_id_mapping:
            source_node_id = node_id_mapping[u]
        elif u_clean in node_id_mapping:
            source_node_id = node_id_mapping[u_clean]
        
        target_node_id = None
        if v in node_id_mapping:
            target_node_id = node_id_mapping[v]
        elif v_clean in node_id_mapping:
            target_node_id = node_id_mapping[v_clean]
        ...
        # 检查是否已有相同关系
        has_relation = any(
            rel["type"] == "TOPOLOGY_DEPENDS_ON" and 
            rel["source"] == source_node_id and 
            rel["target"] == target_node_id
        ...
        if not has_relation:
            relation_data = {
                ...
                "properties": {
                    ...
                    "market_layer_u": rca_analyzer._identify_service_layer(u) or "unknown",
                    "market_layer_v": rca_analyzer._identify_service_layer(v) or "unknown",
                    "created_at": datetime.now(BEIJING_TZ).isoformat()
                }
            }
            kg_data["relationships"].append(relation_data)
    
    # 4.5 CLEANED_ENTITY 关系（原始实体→清洗后实体）
    for original_entity, cleaned_entity in entity_clean_mapping.items():
        if original_entity == cleaned_entity:
            continue
        
        if original_entity not in node_id_mapping or cleaned_entity not in node_id_mapping:
            continue
        
        relation_data = {
            "type": "CLEANED_ENTITY",
            "source": node_id_mapping[original_entity],
            "target": node_id_mapping[cleaned_entity],
            "properties": {
                "cleaning_rule": "Extracted core service name (removed node prefix, port suffix, numeric suffix)",
                "created_at": datetime.now(BEIJING_TZ).isoformat()
            }
        }
        kg_data["relationships"].append(relation_data)
    ...
    kg_file_name = f"market_cluster_{cluster_id}_kg.json"
    kg_file_path = os.path.join(output_dir, kg_file_name)
    
    with open(kg_file_path, 'w', encoding='utf-8') as f:
        json.dump(kg_data, f, ensure_ascii=False, indent=2)
    
    print(f"✅ Market知识图谱已生成: {kg_file_path}")
    return kg_file_path
...
# === 重构的聚类和报告生成逻辑（集成RCA和知识图谱）===
def cluster_and_report(anomalies, output_file, date_online, output_suffix, eps_seconds=300, min_samples=2, args=None):
    if not anomalies:
    ...
    # 创建知识图谱输出目录
    kg_output_dir = os.path.join(os.path.dirname(output_file), "knowledge_graphs", f"{date_online}_{output_suffix}")
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write("📑 Knowledge Graphs: Generated for each cluster in ./knowledge_graphs directory\n")
        f.write("=" * 80 + "\n\n")
        ...
        for idx, cid in enumerate(cluster_ids):
            ...
            kg_output_dir1 = os.path.join(os.path.dirname(output_file), "knowledge_graphs", f"{date_online}_{output_suffix}", f"cluster_{idx+1}")
            
            # 为当前聚类构建知识图谱
            cluster_display_id = idx + 1
            kg_file_path = build_market_knowledge_graph(
                cluster_id=cluster_display_id,
                cluster_anomalies=cluster,
                output_dir=kg_output_dir1,
                rca_analyzer=rca_analyzer
            )

            # 基础聚类信息
            f.write(f"# Cluster #{cluster_display_id}\n")
            ...
            f.write(f"**Knowledge Graph**: {os.path.basename(kg_file_path)}\n")
            ...
            rca_report = rca_analyzer.analyze_with_rca_engine(cluster_display_id, cluster)
            f.write(rca_report)
            f.write("\n")
            f.write("-" * 80 + "\n\n")
        ...
        f.write(f"- Knowledge Graphs: Generated in JSON format with entities, attributes, fault types and relationships\n")

    print(f"✅ Report saved to: {output_file}")
    print(f"📊 Found {len(cluster_ids)} clusters and {len(noise)} isolated anomalies.")
    print(f"📑 Knowledge graphs saved to: {kg_output_dir}")
