...
def main():
    ...
    total_time_steps = offline_data.shape[1]
    feature_dim = offline_data.shape[2]  # 这就是你要的“特征个数”
    ...
    np.save(save_path / "online_data.npy", online_data)
    np.save(save_path / "offline_data.npy", offline_data)

    # ----------------------------
    # 离线数据预处理
    # ----------------------------
    param = {
        "dataset_path": save_path / "offline_data.npy",
        ...
        "mode": 'stand',
        "save_data_path": save_path / "preprocess_data.npy",
    }
    pre.main(param)
    ...
    if addWeight:
        ...
        np.save(save_path / "cmndf.npy", np.ones((num_entity, 25)))
    ...
    # 在线数据预处理（取一天）
    # ----------------------------
    online_data = np.load(save_path / "online_data.npy")
    if dataset_name == 'data1':
        data_1day = online_data[:, 36 * 2:36 * 3, :]
    ...
    param = {
        ...
        "if_feature_scaling": True,
        "mode": 'stand',
        "save_data_path": save_path / "preprocess_onlinedata.npy",
    }
    pre.main(param)

    z = np.load(save_path / "preprocess_onlinedata.npy")
    np.save(save_path / 'z_to_cluster_all_online.npy', z)
    ...
    # ----------------------------
    # 保存测试用 offline_data（转置格式）
    # ----------------------------
    data = np.load(save_path / "offline_data.npy")
    res_list = []
    for i in range(num_entity):
        for j in range(feature_dim):
            res_list.append(data[i, j * 36:(j + 1) * 36, :])
