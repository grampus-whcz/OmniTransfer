...
def get_test_score(data_idx):
    """加载原始异常分数，不涉及任何标签"""
    test_score_path = exp_dir / f'result/{data_idx}/test_score.npy'
    return np.load(test_score_path)


def generate_predictions(calc_latency, percentile=99):
    """
    无监督多变量异常检测：
    - 保留多维结构，每维独立计算阈值
    ...
    pred_dir = exp_dir / f'evaluation_result/predictions'
    ...
    for machine_id in tqdm(data_list, desc=f"Processing machines (percentile={percentile})"):
        # 1. 加载原始分数 (T,) 或 (T, D)
        test_score = get_test_score(machine_id)
    ...
    csv_path = exp_dir / f'evaluation_result/{res_prefix}_machine_best_f1.csv'
