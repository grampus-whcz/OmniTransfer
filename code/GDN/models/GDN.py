...
class GDN(nn.Module):
    ...
    def forward(self, data, org_edge_index):
        ...
        # print(f"edge_index_sets:{len(edge_index_sets)}")
        ...
        for i, edge_index in enumerate(edge_index_sets):
            ...
            dim = weights.shape[-1]
            # topk_num = self.topk  # bug
            topk_num = min(self.topk, cos_ji_mat.size(-1))
            # print(f"cos_ji_mat:{cos_ji_mat.shape}")
            topk_indices_ji = torch.topk(cos_ji_mat, topk_num, dim=-1)[1]
            # print(f"topk_indices_ji:{topk_indices_ji.shape}")

            self.learned_graph = topk_indices_ji
            # print(f"self.learned_graph:{self.learned_graph.shape}")
            ...
            # print(f"batch_gated_edge_index:{batch_gated_edge_index.shape}")
        ...
        # print(f"---- torch.cat(gcn_outs, dim=1){x.shape}")
        x = x.view(batch_num, node_num, -1)
        # print(f"---x.view(batch_num, node_num, -1):{x.shape}")
