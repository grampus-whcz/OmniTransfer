...
def main():
    ...
    for cluster in clusters:
        ...
        for test_id in cluster['test']:
            ...
            elif "init_3_decoder" in exp_key:
                print(f"init_3") 
                new_model.init_param("init_3_decoder")
            ...
            train_data_item, test_data_item = get_data_by_index(test_id, preprocess_train_days)
            # x_train, x_test = preprocess_meanstd(train_data_item.T, test_data_item.T) bug
            x_train, x_test = preprocess_meanstd(train_data_item, test_data_item)
            train_start = time.time()
            new_model.fit([x_train], model_save_dir=model_save_dir, local_epoch=global_epochs//2, valid_portion=0.3)
            
            new_model.restore(model_save_dir)
            if "freeze_12" in exp_key:
                print(f"freeze 12") 
                new_model.unfreeze_layers("freeze_12")
            elif "freeze_1" in exp_key:
                print(f"freeze 1") 
                new_model.unfreeze_layers("freeze_1")
            elif "freeze_encoder" in exp_key:
                print(f"freeze encoder") 
                new_model.unfreeze_layers("freeze_encoder")
            else:
                print(f"unfreeze error")
    ...
    print(f'============== fine-tuning 2 step end ============= \n exp_key: {exp_key} \ntotal_train_time: {total_train_time}')
