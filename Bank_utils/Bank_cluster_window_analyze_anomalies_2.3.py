...
def generate_multi_grain_reports(start_ts, end_ts, cluster_idx, temp_dir):
    ...
    multi_grain_report = []
    multi_grain_report.append(f"Time range: {start_time_short} ~ {end_time_short}")
...
def cluster_and_report(anomalies, output_file, eps_seconds=300, min_samples=2, args=None):
    ...
    # === 修改1：新增字典，用于收集所有集群的多粒度报告 ===
    cluster_multi_grain_json = {}
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for idx, cid in enumerate(cluster_ids):
            ...
            cluster_num = str(idx + 1)  # 集群编号（字符串类型，符合JSON键的要求）
            ...
            cluster_multi_grain_json[cluster_num] = multi_grain_report
        ...
        f.write(f"   Multi-grain reports saved to JSON file (same directory as this report).\n")

    # === 修改3：将收集的多粒度报告序列化为JSON并保存到文件 ===
    # 构建JSON文件路径（与原报告同名，后缀改为.json）
    json_output_file = os.path.splitext(output_file)[0] + "_multi_grain.json"
    try:
        with open(json_output_file, 'w', encoding='utf-8') as json_f:
            # ensure_ascii=False：保留中文等特殊字符；indent=2：格式化输出，便于阅读
            json.dump(cluster_multi_grain_json, json_f, ensure_ascii=False, indent=2)
        print(f"✅ Multi-grain report JSON saved to: {json_output_file}")
    except Exception as e:
        print(f"❌ Failed to save multi-grain JSON file: {e}")
