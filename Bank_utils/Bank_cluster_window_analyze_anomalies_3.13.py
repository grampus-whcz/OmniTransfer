    ...
    def analyze_with_rca_engine(self, cluster_id, anomalies):
        try:
            ...
            try:
                ...
                bank_domain_knowledge_file = "/path/to/config.yaml"
                engine = RCAEngine()
...
def build_knowledge_graph_for_cluster(cluster_id, cluster_anomalies, base_dir, date_str, window_str, output_folder_name):
    """
    增强版知识图谱构建：同时生成JSON、GEXF（可视化）、Cypher（Neo4j导入）三种文件
    """
    # 初始化知识图谱结构
    kg_data = {
        ...
        "time_span": {
            "start": min([a['ts'] for a in cluster_anomalies]) if cluster_anomalies else 0,
            "end": max([a['ts'] for a in cluster_anomalies]) if cluster_anomalies else 0,
            "duration_sec": (max([a['ts'] for a in cluster_anomalies]) - min([a['ts'] for a in cluster_anomalies])) if cluster_anomalies else 0
        },
        "nodes": [],
        "relationships": []
    }
    ...
    # 5. 保存文件
    kg_dir = f"/root/shared-nvme/work/timeSeries/OmniTransfer_new/{output_folder_name}/knowledge_graphs/{date_str}_{window_str}"
    os.makedirs(kg_dir, exist_ok=True)
    
    # 5.1 保存JSON文件
    kg_json_path = f"{kg_dir}/cluster_{cluster_id}/cluster_{cluster_id}_kg.json"
    os.makedirs(f"{kg_dir}/cluster_{cluster_id}", exist_ok=True)
    with open(kg_json_path, 'w', encoding='utf-8') as f:
        json.dump(kg_data, f, ensure_ascii=False, indent=2)
    
    # 5.2 生成并保存GEXF可视化文件
    gexf_path = f"{kg_dir}/cluster_{cluster_id}/cluster_{cluster_id}_visualization.gexf"
    generate_gexf_file(kg_data, node_label_mapping, gexf_path)
    
    # 5.3 生成并保存Neo4j Cypher导入脚本
    cypher_path = f"{kg_dir}/cluster_{cluster_id}/cluster_{cluster_id}_neo4j_import.cypher"
    generate_cypher_script(kg_data, node_label_mapping, cypher_path)
    
    print(f"✅ All knowledge graph files saved to: {kg_dir}")
    print(f"   - JSON: cluster_{cluster_id}_kg.json")
    print(f"   - GEXF: cluster_{cluster_id}_visualization.gexf")
    print(f"   - Cypher: cluster_{cluster_id}_neo4j_import.cypher")
    return kg_json_path
...
def generate_cypher_script(kg_data, node_label_mapping, output_path):
    ...
    # 添加注释头
    cypher_lines.append(f"// Neo4j Import Script for Cluster {kg_data['cluster_id']}")
    ...
    cypher_lines.append(f"MATCH (n) WHERE n.cluster_id = '{kg_data['cluster_id']}' DELETE n;")
    cypher_lines.append("")
...
# === 4. Original Clustering Logic (Updated) ===
...
def cluster_and_report(anomalies, output_file, eps_seconds=300, min_samples=2, args=None):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for idx, cid in enumerate(cluster_ids):
            ...
            # 构建知识图谱（包含JSON/GEXF/Cypher）
            print(f"\n🏗️  Building knowledge graph for Cluster #{idx + 1}...")
            build_knowledge_graph_for_cluster(
                cluster_id=idx + 1,
        ...
        f.write(f"- Knowledge Graphs: Generated for each cluster in /knowledge_graphs/{args.date_online}_{args.output_suffix}/ (JSON/GEXF/Cypher formats)\n")
