...
class ProgrammaticRCAAnalyzer:
    def __init__(self, kg_json_path):
        ...
        self.propagation_paths = [] # Fault propagation paths (new)
    ...
    def _identify_service_layer(self, entity_name):
        """Identify Bank microservice layer for entity (adapted for Bank architecture)"""
    ...
    def _extract_entity_features(self):
        """Extract core features of Bank entities (enhanced version)"""
        ...
        for node in self.kg_data["nodes"]:
            if node["label"] in ["OS", "DOCKER", "DB", "OS_Sub", "DOCKER_Sub", "unknown"]:
                ...
                # Identify Bank microservice layer
        ...
        # 2. Count anomalies, time, severity, duration (enhanced)
    ...
    def _analyze_causal_relationships(self, entities):
        """Analyze causal relationships between entities (new)"""
    ...
    def _calculate_entity_scores(self, entities):
        """Calculate entity root cause scores (enhanced version)"""
        ...
        for entity_id, entity in entities.items():
            ...
            # 5. Weighted total score (enhanced)
            total_score = (
            ...
            self.entity_scores[entity_id] = {
                ...
                # Propagation related
                "propagation_source_count": propagation_source_count,
                "propagation_target_count": propagation_target_count
            }
    
    def _filter_root_causes(self):
        """Filter root causes based on rules (enhanced version)"""
    ...
    def generate_rca_report(self):
        """Generate Bank-specific programmatic RCA report (enhanced version)"""
        ...
        # 2. Build report
        report = []
        report.append(f"# Programmatic Root Cause Analysis Report - Cluster {self.cluster_id} (Bank Microservice)")
        ...
        report.append(f"**Bank Component Weights**: Database(1.0) > Cache(0.95) > Business(0.9) > Governance(0.85) > Gateway(0.8) > Container(0.75) > EntryPoint(0.7)")
        report.append("")
        
        # 3. Fault propagation path analysis (new)
        ...
        # 6. Remediation recommendations (enhanced version, adapted for Bank architecture)
        ...
        if self.root_causes:
            ...
            if primary_cause['component_type'] == 'database':
                report.append(f"5. Check Mysql database: connection pool status, slow queries, lock waits, master-slave synchronization delay")
                report.append(f"6. Verify database performance metrics: QPS, TPS, response time, cache hit rate")
            elif primary_cause['component_type'] == 'cache':
                report.append(f"5. Check Redis cache: memory usage, hit rate, persistence strategy, cluster status")
                report.append(f"6. Verify cache penetration/breakdown/avalanche protection measures are effective")
            elif primary_cause['component_type'] == 'business':
                report.append(f"5. Check Tomcat application: thread pool status, JVM memory, GC status, business log exceptions")
                report.append(f"6. Verify response time and error rate of critical business interfaces")
            ...
            report.append(f"2. Analyze fault propagation paths and optimize Bank microservice call chain dependencies")
            ...
            report.append("1. No clear root cause identified, recommend comprehensive inspection of Bank microservice call chain")
            report.append("2. Focus on health status of Mysql/Redis core data layers")
...
def run_programmatic_analysis(kg_dir, summary_output_path):
    """Run batch programmatic root cause analysis and generate summary"""
    programmatic_summary = {
        "analysis_metadata": {
            ...
            "analysis_type": "Bank_microservice_programmatic_rca_enhanced"
        },
        "clusters": {}
    }
...
# ====================== LLM-driven RCA Analyzer (Enhanced) ======================
class LLMbasedRCAAnalyzer:
    def __init__(self, kg_json_path, llm_config=None):
        """Initialize LLM-driven root cause analyzer (Bank-specific enhanced version)"""
        self.kg_json_path = kg_json_path
        self.kg_data = self._load_kg_data()
        self.cluster_id = self.kg_data["cluster_id"]
        self.total_anomalies = self.kg_data["total_anomalies"]
    ...
    def _convert_kg_to_prompt(self):
        """Convert Bank knowledge graph to optimized LLM prompt (enhanced version)"""
        # 1. System role definition (structured)
        prompt = [
            """### System Role (Must be strictly followed)
You are a senior SRE expert with 10 years of experience in Bank microservice architecture fault diagnosis, proficient in the complete call chain architecture of apache→IG→Tomcat→MG→docker→mysql/redis.
...
Step 3: Causal Reasoning - Identify causal relationships (e.g., DB slow query→API timeout→user complaint);
...
- Focus on anomalies in Mysql/Redis core data layers
...
- Strictly follow the specified output format, no deviation"""
        ...
        # 2. Analysis background (structured)
...
- Bank Microservice Layer Priority: Database(Mysql) > Cache(Redis) > Business(Tomcat) > Governance(MG) > Gateway(IG) > Container(Docker) > EntryPoint(Apache)
- Anomaly Severity Priority: critical(1.0) > major(0.8) > minor(0.6) > warning(0.4) > info(0.2)""")
        
        # 3. Entity metrics (structured table)
        ...
        # 4. Topological dependency relationships (structured)
        ...
        if topology_rels:
            prompt.append("Entity dependency relationship list (Bank call chain):")
        ...
        # 5. Mandatory output format (structured)
        prompt.append("""
...
## 4. Remediation Recommendations (Bank Microservice Specific)
...
3. [Architecture optimization recommendations adapted to Bank call chain]

## 5. Core Data Layer Risk Alert
- Mysql Risk: [Specific risk points for database layer]
- Redis Risk: [Specific risk points for cache layer]
- Overall Risk Level: [High/Medium/Low]""")
    ...
    def generate_rca_report(self):
        """Generate LLM-driven Bank root cause analysis report (enhanced version)"""
        ...
        report = []
        report.append(f"# LLM-driven Root Cause Analysis Report - Cluster {self.cluster_id} (Bank Microservice)")
        report.append(f"**Analysis Time**: {datetime.now(BEIJING_TZ).strftime('%Y-%m-%d %H:%M:%S CST')}")
        ...
        report.append(f"**Bank Architecture**: apache → IG → Tomcat → MG → docker → Mysql/Redis")
    ...
    def save_report(self):
        """Save analysis report (enhanced version with exception handling)"""
...
def run_llm_analysis(kg_dir, api_key, summary_output_path):
    """Run batch LLM-driven root cause analysis (enhanced version)"""
    ...
    llm_summary = {
        "analysis_metadata": {
            ...
            "analysis_type": "Bank_microservice_llm_rca_enhanced"
        },
        "clusters": {}
    }
...
# ====================== Result Fusion and Evaluation Module (New) ======================
def merge_rca_results(programmatic_summary, llm_summary):
    """Fuse programmatic and LLM analysis results"""
    merged_summary = {
        "analysis_metadata": {
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Bank Dataset Root Cause Analysis Program (Enhanced Version)")
    ...
    parser.add_argument("--ground_truth", type=str, default=None,
                        help="Ground truth JSON file path (optional)")
