...
def cluster_and_report(anomalies, output_txt, output_json, eps_seconds=300, min_samples=2):
    ...
    for idx, cid in enumerate(cluster_ids):
        ...
        # 时间跨度
        time_span_str = f"{ts_to_beijing_str(start_ts)} → {ts_to_beijing_str(end_ts)} (Δ = {duration} sec)"
        cluster_data["Time_Span"] = time_span_str
        cluster_data["Total_Anomalies"] = len(cluster)
        ...
        for a in cluster:
            if a['type'] == 'log':
                all_keywords.update(extract_keywords(a['raw']))
        cluster_data["Keywords"] = " ".join(sorted(all_keywords)) if all_keywords else ""
        
        # 异常详情（按类型分类）
        anomalies_detail = defaultdict(list)
        grouped = defaultdict(list)
        for a in cluster:
            grouped[a['type']].append(a)
        ...
        for typ in type_order:
            ...
            for a in grouped[typ]:
                key = (a['entity'], a['attribute'])
                entity_attr_dict[key].append({
                    "timestamp": a['ts'],
                    "time_str": ts_to_beijing_str(a['ts'])
                })
            
            for (ent, attr), ts_list in sorted(entity_attr_dict.items()):
                anomalies_detail[typ].append({
                    "entity": ent,
                    "attribute": attr,
                    "timestamps": ts_list
                })
        
        cluster_data["Anomalies"] = dict(anomalies_detail)
        # 存入JSON
        json_result["clusters"][f"cluster_{idx+1}"] = cluster_data
