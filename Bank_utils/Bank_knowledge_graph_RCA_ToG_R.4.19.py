...
# RCA weight configuration (adapted for Bank microservice architecture)
SCORE_WEIGHTS = {
    "anomaly_count": 0.3,      # Weight for anomaly count
    ...
    "severity_score": 0.1,     # Weight for anomaly severity (new)
    "business_impact": 0.05,   # Weight for business impact (new)
    "causal_propagation": 0.05 # Weight for causal propagation (new)
}

# Bank microservice component base weights (adapted for Bank's layered architecture)
COMPONENT_BASE_WEIGHT = {
    "database": 1.0,    # Mysql - highest priority
    "cache": 0.95,      # Redis - high priority
    "business": 0.9,    # Tomcat - business layer
    "governance": 0.85, # MG - governance layer
    "gateway": 0.8,     # IG - gateway layer
    "container": 0.75,  # Docker - container layer
    "entry_point": 0.7, # Apache - entry layer
    "service_test": 0.65, # Test service
    "unknown": 0.5      # Unknown component
}

# Anomaly severity weight configuration (new)
ANOMALY_SEVERITY_WEIGHT = {
    "critical": 1.0,   # Critical
    "major": 0.8,      # Major
    "minor": 0.6,      # Minor
    "warning": 0.4,    # Warning
    "info": 0.2        # Information
}
...
# Bank Entity Labels (aligned with KG)
TOG_ENTITY_LABELS = {"DB", "OS", "DOCKER", "OS_Sub", "DOCKER_Sub", "unknown"}
...
def _extract_json_object(text):
    ...
    for idx in range(start, len(text)):
        ...
        if char == "{": depth +=1
            ...
            depth -=1
            if depth ==0:
                try: return json.loads(text[start:idx+1])
                except: return None
    return None

# ====================== Bank ToG-R Core Analyzer ======================
class BankToGAnalyzer:
    """ToG-R for Bank Microservice: Low-Token, High Performance RCA"""
    def __init__(self, kg_json_path, kg_data, llm_config=None):
        ...
        for rel in kg_data.get("relationships",[]):
            ...
            if s: self.out_edges[s].append(rel)
            if t: self.in_edges[t].append(rel)

        self.entity_ids = self._collect_entity_ids()
        self.entity_profiles = self._build_entity_profiles()
        self.root_question = "Identify root cause for Bank microservice anomaly using knowledge graph."

    def _identify_service_layer(self, entity_name):
        el = entity_name.lower()
        if 'mysql' in el: return "database"
        elif 'redis' in el: return "cache"
        elif 'tomcat' in el: return "business"
        elif 'mg' in el: return "governance"
        elif 'ig' in el: return "gateway"
        elif 'docker' in el: return "container"
        elif 'apache' in el: return "entry_point"
        else: return "unknown"
    ...
    def _build_entity_profiles(self):
        ...
        for eid in self.entity_ids:
            ...
            p = n.get("properties",{})
            name = p.get("entity_id",eid)
            ctype = self._identify_service_layer(name)
            profiles[eid] = {
                "eid":eid, "name":name, "label":n.get("label"),
                "component_type": ctype,
                "is_main": p.get("is_main_entity",True),
                "anomaly_count":0, "severity_score":0.0,
                "first_ts":float("inf"), "last_ts":0,
                "fault_types":set(), "neighbors":set(), "biz":0,
                "evidence":[], "duration":0.0
            }

        for rel in self.kg_data.get("relationships",[]):
            t = rel.get("type")
            s,tar,p = rel.get("source"), rel.get("target"), rel.get("properties",{})
            if t=="HAS_ANOMALY" and s in profiles:
                ...
                e["duration"] += max(0, p.get("end_ts",ts)-p.get("start_ts",ts))
                e["evidence"].append(f"{p.get('metric','anom')} {sev}")
            elif t=="HAS_ATTRIBUTE" and s in profiles:
                fn = self.nodes_by_id.get(tar,{}).get("properties",{}).get("fault_type")
                if fn: profiles[s]["fault_types"].add(fn)
        ...
        for e in profiles.values():
            if e["first_ts"]==float("inf"): e["first_ts"]=None
            e["fault_types"] = sorted(e["fault_types"])
        return profiles

    def _entity_sort_key(self, eid):
        p = self.entity_profiles[eid]
        ts = p["first_ts"] or float("inf")
        return (-p["anomaly_count"], -p["severity_score"], ts, -len(p["neighbors"]))
    ...
    def _available_relation_keys(self, eid):
        r = set()
        for e in self.out_edges.get(eid,[]): r.add(f"out::{e.get('type')}")
        for e in self.in_edges.get(eid,[]): r.add(f"in::{e.get('type')}")
        return sorted(r)

    def _expand_relation(self, eid, rkey):
        if "::" not in rkey: return []
        d,t = rkey.split("::",1)
        edges = self.out_edges[eid] if d=="out" else self.in_edges[eid]
        res = []
        for rel in edges:
            if rel.get("type")!=t: continue
            s = eid if d=="out" else rel.get("source")
            tar = rel.get("target") if d=="out" else eid
            nxt = tar if tar in self.entity_profiles else (s if s in self.entity_profiles else None)
            res.append({
                "rkey":rkey, "triplet":f"({self._entity_snapshot(s)})-[{t}]->({self._entity_snapshot(tar)})",
                "next":nxt
            })
        return res

    def _prompt_global_relation_prune(self, keys):
...
"""
    ...
    def _parse_relation_scores(self, txt, keys):
        ...
        allowed = set(keys)
        res = []
        for k,s in re.findall(pat, txt or ""):
            k = k.strip()
            if k in allowed: res.append((k, float(s)))
        if not res: return [(k,1/3) for k in keys[:3]]
    ...
    def _get_entity_scores(self, candidates):
        ...
        for c in candidates:
            ...
            w = COMPONENT_BASE_WEIGHT.get(p["component_type"],0.5)
            score = p["anomaly_count"]*0.4 + p["severity_score"]*0.3 + (1 if p["is_main"] else 0.2)*w
...
Output:
{{
  "root_cause_ranking": [{{"entity_id":"","confidence":0.0,"reason":"","fault_types":[],"first_anomaly_time":""}}],
  "fault_propagation": "",
  "summary": ""
}}"""

    def _llm(self, prompt):
        msg = [{"role":"system","content":"RCA expert for Bank microservice."},{"role":"user","content":prompt}]
    ...
    def run(self):
        ...
        states = self._seed()
        chains, trace, llm_trace = [], [], []

        for d in range(1, TOG_SEARCH_CONFIG["depth"]+1):
            all_r = set()
            for s in states:
                for r in self._available_relation_keys(s["entity_id"]):
                    all_r.add(r)
            all_r = sorted(list(all_r))[:6]
            if not all_r: break

            r_resp = self._llm(self._prompt_global_relation_prune(all_r))
            print(f"Sleep 20 seconds after all relations prune LLM call at depth {d}")
            time.sleep(20)
            llm_trace.append({"stage":"rel_prune","depth":d,"resp":r_resp})
            selected = self._parse_relation_scores(r_resp.get("content"), all_r)

            next_states = []
            for s in states:
                eid = s["entity_id"]
                for rk, rw in selected:
                    cs = self._expand_relation(eid, rk)
                    if not cs: continue
                    es = self._get_entity_scores(cs)
                    for i,c in enumerate(cs):
                        nid = c["next"] or eid
                        if c["next"] and c["next"] in s["vis"]: continue
                        scr = s["score"] * rw * es[i]
                        next_states.append({
                            "entity_id":nid, "score":scr,
                            "path":s["path"]+[c["triplet"]],
                            "vis":s["vis"]+([nid] if c["next"] else [])
                        })
            ...
            for s in sorted(next_states, key=lambda x:x["score"], reverse=True):
                ...
                if k not in uniq: uniq[k]=s
            states = list(uniq.values())[:TOG_SEARCH_CONFIG["width"]]

            suf = self._llm(self._prompt_sufficiency(chains))
            print(f"Sleep 20 seconds after chains prune LLM call at depth {d}")
            time.sleep(20)
            llm_trace.append({"stage":"suf","depth":d,"resp":suf})
            if suf.get("content","").strip().lower() in ["yes","{yes}"]: break

        final = self._llm(self._prompt_final(states, chains))
        print(f"Sleep 20 seconds after final reasoning LLM call")
        time.sleep(20)
        llm_trace.append({"stage":"final","resp":final})
        j = _extract_json_object(final.get("content"))
        ...
        if not rank:
            ...
            for i,eid in enumerate(fallback,1):
                p = self.entity_profiles[eid]
                rank.append({
                    "rank":i, "entity_id":eid, "entity_name":p["name"],
                    "component_type":p["component_type"], "confidence":round(1-0.2*(i-1),4),
                    "reason":"ToG-R fallback", "fault_types":p["fault_types"],
                    "first_anomaly_time":self._fmt_ts(p["first_ts"]), "anomaly_count":p["anomaly_count"]
                })

        res = {
            "analysis_method":"tog-r (bank)",
            "cluster_id":self.cluster_id, "model_used":self.llm_config["model"],
            "primary_root_cause":rank[0], "top_5_root_causes":rank[:5],
            "reasoning_chains":chains[:15],
            "fault_propagation":(j or {}).get("fault_propagation",""),
            "summary":(j or {}).get("summary",""),
            "llm_trace":llm_trace, "final_response":final
        }
...
class ProgrammaticRCAAnalyzer:
    ...
    def _extract_entity_features(self):
        ...
        for node in self.kg_data["nodes"]:
            if node["label"] in TOG_ENTITY_LABELS:
                eid = node["id"]
                name = node["properties"].get("entity_id",eid)
                is_main = node["properties"].get("is_main_entity",True)
                ctype = self._identify_service_layer(name)
                entities[eid] = {
                    "id":eid,"name":name,"ctype":ctype,"is_main":is_main,
                    "anomaly_count":0,"first_ts":float("inf"),"last_ts":0,
                    "fault_types":set(),"neighbors":set(),"cw":COMPONENT_BASE_WEIGHT.get(ctype,0.5),
                    "sev":0.0,"dur":0.0,"biz":0
                }
        ...
        for rel in self.kg_data["relationships"]:
            if rel["type"]=="HAS_ANOMALY":
                ...
                if eid in entities:
                    ...
                    e["sev"]+=ANOMALY_SEVERITY_WEIGHT.get(sev,0.2)
                    s=rel["properties"].get("start_ts",ts)
                    e["dur"]+=max(0,rel["properties"].get("end_ts",ts)-s)
        ...
        for rel in self.kg_data["relationships"]:
            if rel["type"]=="HAS_ATTRIBUTE":
                ...
                if s in entities and t in attr_map:
                    entities[s]["fault_types"].add(attr_map[t])

        for rel in self.kg_data["relationships"]:
            if rel["type"]=="TOPOLOGY_DEPENDS_ON":
                s,t=rel["source"],rel["target"]
                if s in entities: entities[s]["neighbors"].add(t)
                if t in entities: entities[t]["neighbors"].add(s)
        for rel in self.kg_data["relationships"]:
            if rel["type"]=="IMPACTS_BUSINESS":
                s=rel["source"]
                if s in entities: entities[s]["biz"]+=1
        return entities
        ...
        for ts in sorted(timeline.keys()):
            ...
            for e in cur:
                if e not in prev and e in entities:
                    common = entities[e]["neighbors"] & prev
                    if common:
    ...
    def _calculate_entity_scores(self, entities):
        max_cnt = max([e["anomaly_count"] for e in entities.values()],default=1) or 1e-6
        ts_list = [e["first_ts"] for e in entities.values() if e["first_ts"]!=float("inf")]
        min_ts, max_ts = (min(ts_list),max(ts_list)) if ts_list else (0,1)
        if max_ts-min_ts<1e-6: max_ts=min_ts+1
        max_nei = max([len(e["neighbors"]) for e in entities.values()],default=1) or 1e-6
        max_sev = max([e["sev"] for e in entities.values()],default=1) or 1e-6
        max_dur = max([e["dur"] for e in entities.values()],default=1) or 1e-6
        max_biz = max([e["biz"] for e in entities.values()],default=1) or 1e-6
        max_cw = max(COMPONENT_BASE_WEIGHT.values())

        self._analyze_causal_relationships(entities)
        causal = defaultdict(float)
        for p in self.propagation_paths: causal[p["source"]]+=0.1

        for eid,e in entities.items():
            if not e["is_main"]: continue
            cnt_s = e["anomaly_count"]/max_cnt
            time_s = (max_ts-e["first_ts"])/(max_ts-min_ts) if e["first_ts"]!=float("inf") else 0
            topo_s = len(e["neighbors"])/max_nei
            comp_s = e["cw"]/max_cw
            sev_s = e["sev"]/max_sev
            biz_s = e["biz"]/max_biz
            causal_s = min(causal.get(eid,0),0.5)

            total = (cnt_s*0.3 + time_s*0.15 + topo_s*0.2 + comp_s*0.15 + sev_s*0.1 + biz_s*0.05 + causal_s*0.05)
            total = max(0,min(1,total))

            ft = self._fmt_ts(e["first_ts"]) if e["first_ts"]!=float("inf") else "N/A"
            self.entity_scores[eid] = {
                "entity_id":eid,"entity_name":e["name"],"component_type":e["ctype"],
                "total_score":round(total,4),"anomaly_count":e["anomaly_count"],
                "first_anomaly_time":ft,"fault_types":sorted(e["fault_types"]),
                "confidence":round(total,4),"reason":"Programmatic rule-based"
            }
    ...
    def _filter_root_causes(self):
        ...
        if not self.root_causes and sorted_ents: self.root_causes.append(sorted_ents[0])
...
def merge_rca_results(prog, llm):
    ...
    for cid in prog["clusters"]:
        ...
        merged["clusters"][cid] = {"top_5_merged_root_causes":sorted_res[:5]}
    return merged

def evaluate_rca_results(merged, gt_path=None):
    eval = {"metrics":{"top_1_accuracy":0,"top_3_accuracy":0}}
    if not gt_path or not os.path.exists(gt_path): return eval
    with open(gt_path) as f: gt = json.load(f)
    cnt=0; c1=0; c3=0
    for cid, r in merged["clusters"].items():
        if cid not in gt: continue
        cnt+=1
        true = gt[cid]["root_cause_entity"]
        top = [x[0] for x in r["top_5_merged_root_causes"]]
        if top and top[0]==true: c1+=1
        if true in top[:3]: c3+=1
    if cnt>0:
        eval["metrics"]["top_1_accuracy"]=round(c1/cnt,2)
        eval["metrics"]["top_3_accuracy"]=round(c3/cnt,2)
    return eval
    ...
    for r,d,files in os.walk(kg_dir):
        for f in files:
            if f.endswith("_kg.json"):
                ...
                _, s = a.generate_rca_report()
                summary["clusters"][os.path.basename(r)] = s
    with open(out_path,"w") as f: json.dump(summary,f,indent=2)
    return summary

def run_llm_analysis(kg_dir, _, out_path):
    summary = {"clusters":{}}
    for r,d,files in os.walk(kg_dir):
        for f in files:
            if f.endswith("_kg.json"):
                path = os.path.join(r,f)
                a = LLMbasedRCAAnalyzer(path)
                _, s = a.generate_rca_report()
                a.save_report()
                summary["clusters"][os.path.basename(r)] = s
    with open(out_path,"w") as f: json.dump(summary,f,indent=2)
...
# ====================== Main ======================
def main():
    parser = argparse.ArgumentParser(description="Bank ToG-R RCA")
    ...
    log_file = open(f"{base_dir}/Bank_cluster_window_anomaly_report_{args.date_online}_{args.output_suffix}_knowledge_graph.log", 'a', encoding='utf-8')
    ...
    # 4. Evaluate results (new, optional)
