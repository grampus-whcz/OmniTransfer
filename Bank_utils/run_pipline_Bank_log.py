...
def main(args):
    ...
    offline_src = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Bank/log/temp_data/raw_data/train_valid/{args.date_offline}/raw_log/service_log_patterns_count.npy"
    )
    online_src = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Bank/log/temp_data/raw_data/train_valid/{args.date_online}/raw_log/service_log_patterns_count.npy"
    )
    log_meta_file = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Bank/log/temp_data/raw_data/train_valid/{args.date_online}/raw_log/service_log_patterns_count_meta.npy"  
    )
    log_template_file = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Bank/log/temp_data/analysis/log/log_istio_patterns.json"  
    )
    ...
    analyze_script = os.path.join(base_dir, "Bank_utils/Bank_log_analyze_anomalies.py")
    output_file = f"{base_dir}/{args.output_folder_name}/Bank_log_anomalies_{args.date_online}_{args.output_suffix}.npy"

    cmd_analyze = (
        f"/root/shared-nvme/.conda/envs/OmniTransfer_py3.7/bin/python {analyze_script} "
        f"--log_meta {log_meta_file} "
