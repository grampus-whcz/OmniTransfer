...
def main(args):
    ...
    offline_src = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Bank/metric/metric_app_{args.date_offline}_60s.npy"
    )
    online_src = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Bank/metric/metric_app_{args.date_online}_60s.npy"
    )
    metric_app_meta_file = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Bank/metric/metadata_app_{args.date_online}_60s.json"
    )
    ...
    analyze_script = os.path.join(base_dir, "Bank_utils/Bank_metric_app_analyze_anomalies.py")
    output_file = f"{base_dir}/{args.output_folder_name}/Bank_metric_app_anomalies_{args.date_online}_{args.output_suffix}.npy"
