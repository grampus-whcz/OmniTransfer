...
#             # Step 2: Filter concentrated anomalies - FIXED: Use 'timestamp' column consistently
#             all_ts = []
...
#             T0 = min(all_ts)
#             T_window = T0 + CONCENTRATION_WINDOW_MINUTES * 60
...
#             df_enriched = self.enrich_candidate_features(df_candidate, df_node_window, df_edge_window, G, T0, T_window)
...
#     def enrich_candidate_features(self, df_candidate, df_node_window, df_edge_window, G, T0, T_window):
...
#         """Generate LLM-friendly RCA report with dependency analysis - FIXED: 新增属性列示"""
#         try:
...
#             root_attrs_str = ', '.join([f"{k}({v} times)" for k, v in root_attrs.items()]) if root_attrs else "无"
...
#                     attrs_str = ', '.join([f"{k}({v} times)" for k, v in anomaly_attrs.items()]) if anomaly_attrs else "无"
#                     # 拼接展示内容
...
#             if earliest_ts and not np.isinf(earliest_ts):
#                 report.append(f"3. Investigate anomalies starting at {earliest_str} for {root_cause}")
...
#         top_attrs_str = ', '.join([f"{k}({v} times)" for k, v in top_entity_attrs.items()]) if top_entity_attrs else "无"
