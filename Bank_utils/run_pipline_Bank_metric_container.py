...
def main(args):
    ...
    offline_src = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Bank/metric/metric_container_{args.date_offline}_60s.npy"
    )
    online_src = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Bank/metric/metric_container_{args.date_online}_60s.npy"
    )
    metric_app_meta_file = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Bank/metric/metadata_container_{args.date_online}_60s.json"
    )
    ...
    analyze_script = os.path.join(base_dir, "Bank_utils/Bank_metric_container_analyze_anomalies.py")
    output_file = f"{base_dir}/{args.output_folder_name}/Bank_metric_container_anomalies_{args.date_online}_{args.output_suffix}.npy"

    cmd_analyze = (
        f"/root/shared-nvme/.conda/envs/OmniTransfer_py3.7/bin/python {analyze_script} "
        f"--meta {metric_app_meta_file} "
