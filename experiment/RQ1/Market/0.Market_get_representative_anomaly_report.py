...
def write_section(f, groups, section_title, is_log=False, templates=None):
    ...
    f.write("=" * 80 + "\n\n")
    if not groups:
        f.write("No anomalies found in the time window.\n\n")
        return

    # 根据是否为 log 类型选择排序依据
    keys = sorted(set(key[0] for key in groups.keys())) if is_log else sorted(groups.keys())
    for key in keys:
        if is_log:
            pod= key
            f.write(f"Pod: {pod}\n")
            ...
            (entity, attr), timestamps = key, groups[key]
            timestamps_sorted = sorted(timestamps)
            count = len(timestamps_sorted)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate anomaly report for Market dataset.")
