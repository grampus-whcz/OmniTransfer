...
CONFIGS = {
    ...
    "API_BASE": "https://api2.qiandao.mom/v1",
    "API_KEY": os.getenv("MODELSCOPE_API_KEY", "sk-8CwQhXLMcCdb3HtdH4oiMkf8IDM3k9G3OYM0uPUvFJ1YzqmV")
}

GROUNDTRUTH_PATH = "/root/shared-nvme/work/timeSeries/OmniTransfer_new/experiment/RQ1/Telecom/groundtruth.csv"
REPORTS_DIR = Path("/root/shared-nvme/work/timeSeries/OmniTransfer_new/experiment/RQ1/Telecom/report")  # ← 已更新！
OUTPUT_JSONL = "telecom_root_cause_reports_en_6mtd.jsonl"
...
def extract_metrics(texts):
    ...
    metric_keywords = [
        ...
        'TCP', 'FIN-WAIT', 'CLOSE-WAIT', 'TIME-WAIT', 'load', 'utilization',
        'response time', 'success rate', 'error rate', 'rr', 'sr', 'rt',
        'connection', 'packet loss', 'I/O', 'thread', 'GC', 'heap',
        'file descriptor', 'socket', 'bandwidth', 'retransmit',
        'OSLinux_NETWORK', 'DBCP2', 'HTTP', 'gRPC', 'trace'
    ]
    found = set()
    combined_text = " ".join(str(t) for t in texts).lower()
    for kw in metric_keywords:
        if kw.lower() in combined_text:
            # 保留原始大小写形式（简化处理：用关键词本身）
            found.add(kw)
    return sorted(found)
...
def main():
    ...
    with open(OUTPUT_JSONL, 'w', encoding='utf-8') as out_f:
        for idx, row in enumerate(rows, 1):
            ...
            try:
                ...
                # 提取用于 metrics 的所有文本
                all_texts_for_metrics = [row['reason']]
                if result and isinstance(result, dict):
                    all_texts_for_metrics.extend(result.get("Evidence", []))
                    all_texts_for_metrics.append(result.get("RootCause", ""))

                metrics_list = extract_metrics(all_texts_for_metrics)

                # 构建兼容 RAG 脚本的记录（AWS-style schema + component & metrics）
                if result and isinstance(result, dict):
                    ...
                    record = {
                        ...
                        "component": row['component'],  # 👈 显式添加 component
                        "metrics": metrics_list,        # 👈 添加 metrics 列表
                        "symptoms": [row['reason']] + [str(e)[:200] for e in evidence_list[:3]],
                    ...
                    record = {
                        "original_source": f"{row['component']} failure at {row['datetime']}",
                        "system_type": f"Banking Microservice ({row['level']})",
                        "component": row['component'],  # 👈 fallback 也包含 component
                        "metrics": metrics_list,        # 👈 即使失败也尝试提供 metrics
                        "symptoms": [row['reason']],
                ...
                out_f.flush()
                print("✅ Wrote RAG-compatible record with component and metrics")

            except Exception as e:
                print(f"💥 API call failed: {e}")
                # Still write a minimal valid record to avoid empty chunks
                fallback_metrics = extract_metrics([row['reason']])
                fallback_record = {
                    "original_source": f"{row['component']} failure at {row['datetime']}",
                    "system_type": f"Banking Microservice ({row['level']})",
                    "component": row['component'],
    ...
    print(f"\n✅ All results saved to {OUTPUT_JSONL} (RAG-compatible format with component & metrics)")
