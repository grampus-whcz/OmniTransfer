...
def write_metric_A_section(f, groups):
    ...
    # 只保留出现 ≥3 次的 (entity, attr)
    frequent_groups = {k: v for k, v in groups.items() if len(v) >= 2}
    
    if not frequent_groups:
        f.write("No frequent metric app anomalies (≥2 occurrences) found in the time window.\n\n")
        return

    entities = sorted(set(k[0] for k in frequent_groups.keys()))
    for ent in entities:
        f.write(f"Entity: {ent}\n")
        ent_groups = [(k, v) for k, v in frequent_groups.items() if k[0] == ent]
...
def write_metric_B_section(f, groups):
    ...
    if not frequent_groups:
        f.write("No frequent metric container anomalies (≥2 occurrences) found in the time window.\n\n")
        return
    ...
    for ent in entities:
        f.write(f"Entity: {ent}\n")
        ent_groups = [(k, v) for k, v in frequent_groups.items() if k[0] == ent]
        ent_groups.sort(key=lambda x: x[0][1])
        for (entity, attr), timestamps in ent_groups:
            timestamps_sorted = sorted(timestamps)
            count = len(timestamps_sorted)
            f.write(f"  - Attribute '{attr}': {count} anomalies at timestamps:\n")
...
def write_trace_section(f, groups):
    ...
    if not frequent_groups:
        f.write("No frequent trace anomalies (≥2 occurrences) found in the time window.\n\n")
        return

    edges = sorted(set(k[0] for k in frequent_groups.keys()))
    for edge in edges:
        f.write(f"Edge: {edge}\n")
        edge_groups = [(k, v) for k, v in frequent_groups.items() if k[0] == edge]
        edge_groups.sort(key=lambda x: x[0][1])
...
def main(date_str, time_window_str, target_ts, output_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:        
        ...
        f.write("💡 Note: 'CST' = China Standard Time (UTC+8). Only anomalies with ≥3 occurrences are reported.\n")

    print(f"✅ Report (only ≥3 anomalies) written to: {output_file}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Telecom anomaly report (only frequent anomalies ≥3).")
