...
def main(args):
    ...
    # 构造 Telecom Metric A 文件路径
    offline_src = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Telecom/metric/entity_A_{args.date_offline}_{args.bucket_sec}s.npy"
    )
    online_src = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Telecom/metric/entity_A_{args.date_online}_{args.bucket_sec}s.npy"
    )
    metric_meta_file = os.path.join(
        base_dir,
        f"OpenRCA_preprocess_dataset/Telecom/metric/metadata_A_{args.date_online}_{args.bucket_sec}s.json"
    )
    ...
    # Step 1 & 2: 复制数据文件（覆盖 dataset/data1/）
    run_cmd(f"cp {offline_src} {dst_offline}")
    run_cmd(f"cp {online_src} {dst_online}")

    # Step 3: 聚类（必须运行，为 OmniTransfer 提供 cluster_result.npy）
    ...
    # Step 5: 分析 Telecom Metric A 异常
    analyze_script = "/root/shared-nvme/work/timeSeries/OmniTransfer_new/Telecom_utils/7.Telecom_metric_A_analyze_anomalies.py"
    output_file = f"{base_dir}/{args.output_folder_name}/Telecom_metric_A_anomalies_{args.date_online}_{args.output_suffix}.npy"
    ...
    print(f"\n✅ Telecom Metric A Pipeline completed. Anomaly results saved to: {output_file}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run full OmniTransfer anomaly detection pipeline for Telecom Metric A.")
    ...
    parser.add_argument("--method", type=str, default="TranAD",
                        help="Anomaly detection method name used in run.sh, e.g., 'TranAD', 'OmniAnomaly', etc.")
    ...
    parser.add_argument("--output_suffix", type=str, default="result",
                        help="Suffix for output anomalies file, e.g., '00_to_06' → anomalies_00_to_06.npy")
